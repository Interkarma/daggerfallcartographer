...
// Location PreRecord header
typedef struct _LOCATION_PRERECORDHEADER
{
	long			PreRecordCount;					// Number of 6-byte prerecords that follow
	unsigned char	Data;							// PreRecords - Unknown purpose
} LOCATION_PRERECORDHEADER, *LPLOCATION_PRERECORDHEADER;

// Location PreRecord Data
typedef struct _LOCATION_PRERECORD
{
	short			PostRecordIndex;				// Locations = variable, Dungeons = 0xffff
	char			Unknown[4];
} LOCATION_PRERECORD, *LPLOCATION_PRERECORD;

// Location header
typedef struct _LOCATION_HEADER
{
	long			OneValue1;						// Always 1
	short			NullValue1;						// Always 0
	char			NullValue2;						// Always 0
	long			XPos;							// XPos in game units
	long			NullValue3;						// Always 0
	long			YPos;							// YPos in game units
	long			Type;							// Locations = 0x8000, Dungeons = 0x00
	long			Unknown1;
	long			Unknown2;
	short			OneValue2;						// Always 1
	short			LocationID;						// Used for quests?
	long			NullValue4;						// Always 0
	short			Unknown3;						// Locations = 0, Dungeons = 1
	long			Unknown4;
	char			NullValue5[26];					// This range always zero
	char			Name[32];						// Name string
	char			Unknown5[9];
	short			PostRecordCount;				// Number of post records
} LOCATION_HEADER, *LPLOCATION_HEADER;

// Location PostRecord Header
typedef struct _LOCATION_POSTRECORDHEADER
{
	char			Unknown1[5];
	char			Data;							// Post Records
} LOCATION_POSTRECORDHEADER, *LPLOCATION_POSTRECORDHEADER;

// Location PostRecord Data
typedef struct _LOCATION_POSTRECORD
{
	short			HouseNameType;					// Affects name generation
	char			NullValue1[16];					// Always 0
	short			Unknown1;
	short			Unknown2;
	short			LocationID;
	char			HouseType;						// To do with house name / type?
	char			Unknown3;
} LOCATION_POSTRECORD, *LPLOCATION_POSTRECORD;

// Location Description
typedef struct _LOCATION_DESCRIPTION
{
	char			AnotherName[32];				// Another name for this location
	long			Unknown1;
	long			Unknown2;
	char			BlockWidth;						// Width in blocks (1-8)
	char			BlockHeight;					// Height in block (1-8)
	char			Unknown3[7];
	char			BlockFileIndex[64];				// File index of block
	char			BlockFileNumber[64];			// File number of block
	char			BlockFileChar[64];				// File character of block
	char			Unknown4[32];
	char			Unknown5;
	char			Unknown6;
	char			NullValue1[19];					// Always 0
	long			Unknown7[22];
	char			NullValue2[40];					// Always 0
	long			Unknown8;
} LOCATION_DESCRIPTION, *LPLOCATION_DESCRIPTION;

// Region MapTable
typedef struct _REGION_MAPTABLE
{
	long			Unknown1;
	char			Unknown2;
	long			XPosAndType;					// Bits 0-16 = XPos, Bits 17-31 = Type
	short			Ypos;							// YPos on travel map
	short			Unknown3;
	long			Unknown4;
} REGION_MAPTABLE, *LPREGION_MAPTABLE;

// Region MapNames
typedef struct _REGION_MAPNAMES
{
	long			nCount;							// Number of locations in this region
	char			Data;							// Start of [nCount][32] name array
} REGION_MAPNAMES, *LPREGION_MAPNAMES;
...
static char *FileIndices[] = {
	{"TVRN????.RMB"}, {"GENR????.RMB"}, {"RESI????.RMB"}, {"WEAP????.RMB"},
	{"ARMR????.RMB"}, {"ALCH????.RMB"}, {"BANK????.RMB"}, {"BOOK????.RMB"},
	{"CLOT????.RMB"}, {"FURN????.RMB"}, {"GEMS????.RMB"}, {"LIBR????.RMB"},
	{"PAWN????.RMB"}, {"TEMP????.RMB"}, {"TEMP????.RMB"}, {"PALA????.RMB"},
	{"FARM????.RMB"}, {"DUNG????.RMB"}, {"CAST????.RMB"}, {"MANR????.RMB"},
	{"SHRI????.RMB"}, {"RUIN????.RMB"}, {"SHCK????.RMB"}, {"GRVE????.RMB"},
	{"FILL????.RMB"}, {"KRAV????.RMB"}, {"KDRA????.RMB"}, {"KOWL????.RMB"},
	{"KMOO????.RMB"}, {"KCAN????.RMB"}, {"KFLA????.RMB"}, {"KHOR????.RMB"},
	{"KROS????.RMB"}, {"KWHE????.RMB"}, {"KSCA????.RMB"}, {"KHAW????.RMB"},
	{"MAGE????.RMB"}, {"THIE????.RMB"}, {"DARK????.RMB"}, {"FIGH????.RMB"},
	{"CUST????.RMB"}, {"WALL????.RMB"}, {"MARK????.RMB"}, {"SHIP????.RMB"},
	{"WITC????.RMB"}
};
