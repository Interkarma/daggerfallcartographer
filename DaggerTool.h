...
// Location layout
typedef struct _LOCATION_BLOCKLAYOUT
{
	long	nWidth;				// Width in blocks (1-8)
	long	nHeight;			// Height in blocks (1-8)
	long	Tiles[64];			// Index to blocks
} LOCATION_BLOCKLAYOUT, *LPLOCATION_BLOCKLAYOUT;
...
public:		// methods
	...
	long ResolveBlockID( LPCSTR pszObjectID );
...
private:	// data
	...
	THashArray<UINT, 743>		m_haObjectID;
};
...
public:		// methods
	bool	Open( LPCSTR pszPath, BOOL bReadOnly = TRUE );
	void	Close();
	bool	OpenRegion( long nRegion );
	void	CloseRegion();
	bool	IsRegionOpen() {return m_bIsRegionOpen;}

	long	GetLocationCount();
	bool	GetLocationName( long nLocation, char* pszBuffer );
	long	GetLocationType( long nLocation );
	LPLOCATION_DESCRIPTION GetLocationDescription( long nLocation );

	bool	GetLocationBlockLayout( long nLocation, BSABlocks *pBlocks, LPLOCATION_BLOCKLAYOUT pBlockLayout );
	long	ResolveBlockIndex( BSABlocks *pBlocks, LPLOCATION_DESCRIPTION pDesc, int x, int y );

private:	// data
	bool	m_bIsRegionOpen;
	long	m_nCurRegion;
	char*	m_pData_MapPItem;
	char*	m_pData_MapDItem;
	char*	m_pData_MapTable;
	char*	m_pData_MapNames;
