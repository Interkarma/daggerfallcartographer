...
typedef struct _ALCHEMY_CREATESTRUCT
{
	...
	D3DXVECTOR3 vDirectionalLight;		// Direction of directional light
	D3DCOLOR	colAmbientLight;		// Colour and intensity of ambient lighting
	D3DCOLOR	colDirectionalLight;	// Colour and intensity of directional light
...
struct AcResource
{
	...
	LPARAM			lParam;			// 32-bit parameter for this object
};
...
public:		// data
	...
	BOOL			bSelected;			// TRUE if this sprite is selected (highlighted), otherwise FALSE
	...
	D3DCOLOR		selectedColour;		// Selected colour of this sprite
};
...
// TODO: SceneNode description
typedef struct _ACSCENE_NODE
{
	_ACSCENE_NODE*	pParent;				// Pointer to parent node
	D3DXVECTOR3		vPosition;				// Offset position of node within parent
	D3DXVECTOR3		vSize;					// Node dimensions
} ACSCENE_NODE, *LPACSCENE_NODE;
...
public:		// Engine management methods
	...
	DWORD	GetLastFramePolyCount() {return m_dwLastFramePolyCount;}
...
public:		// Sprite management methods
	...
	UINT	HitTestSprites( int x, int y );
	void	SelectSprite( UINT nSprite, BOOL bSelect = TRUE );
	BOOL	IsSpriteSelected( UINT nSprite );
	void	SelectAllSprites( BOOL bSelect = TRUE );
	DWORD	GetSelectedSpriteCount();
...
public:		// Resource management methods
	UINT	InstallMeshFromMemory( TFlexibleMesh* pMesh, LPCSTR pszIndex = NULL, LPARAM lParam = NULL );
	UINT	InstallMeshFromFile( LPCSTR pszPath, LPCSTR pszIndex = NULL, LPARAM lParam = NULL );
	UINT	InstallTextureFromMemory( UINT nAppend, LPRECT pSrcRect, DWORD dwSrcPitchInPixels, LPVOID pBits, DWORD dwBitDepth = AC_DEFAULT, LPCSTR pszIndex = NULL, DWORD dwColorKey = 0xff000000, LPARAM lParam = NULL );
	//UINT	InstallTextureFromFile( LPCSTR pszPath, DWORD dwBitDepth = AC_DEFAULT, LPCSTR pszIndex = NULL, DWORD dwColorKey = 0xff000000, LPARAM lParam = NULL );
	BOOL	GetTextureRect( UINT nTexture, LPRECT pRectOut );
	LPARAM	GetResourceParam( UINT nResource );
...
public:		// Display and camera management methods
	...
	BOOL	CameraThrust( UINT nCamera, float fRight, float fUp, float fFront, bool bDolly = false );

public:		// Export methods
	BOOL	SaveImageToFile( LPCSTR pszPath, DWORD dwFormat, LPRECT pSrcRect, DWORD dwSrcPitchInPixels, LPVOID pBits );
...
private:	// methods
	...
	UINT	InstallResource( DWORD dwType, LPCSTR pszIndex, LPARAM lParam );
	void	DestroyResource( UINT nResource );
	BOOL	InstallTexture( UINT nAppend, LPRECT pSrcRect, DWORD dwSrcPitchInPixels, LPVOID pBits, DWORD dwBitDepth = AC_DEFAULT, LPCSTR pszIndex = NULL, DWORD dwColorKey = 0xff000000, LPARAM lParam = NULL );
	...
	BOOL	ComputeMeshNormals( TFlexibleMesh *pMeshIn );
...
private:	// Engine management data
	...
	DWORD		m_dwLastFramePolyCount;
	DWORD		m_dwSelectedSpriteCount;
	float		m_fTI;
	float		m_fFPS;
...
private:	// Engine state data
	...
	D3DCOLOR	m_colDirectionalLight;
